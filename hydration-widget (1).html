...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  body::before {
    ...
    background:
      radial-gradient(ellipse 80% 60% at 50% 110%, rgba(14,165,233,0.18) 0%, transparent 70%),
  ...
  /* Waves are static — no scroll animation */
  ...
  /* Custom number input: hide native spinner, use ↑↓ buttons */
  .num-wrap {
...
<div class="widget" id="widget">
  ...
  <!-- HEADER -->
  <div class="header">
    <div class="title">Hidratação</div>
  ...
  <div class="vessel-wrap">
    <div class="vessel-svg-wrap">
      ...
      <!--
        ...
        BOTTLE (full height):
          ...
          Shoulder: diagonal lines neck→body
        ...
        Waves: outer <g> translateY to waterline (SVG attr updated by JS)
               inner <g> CSS animation translateX only → no conflict
      -->
        ...
        <g id="shapeBottle">
          ...
          <g clip-path="url(#bottleClip)">
            ...
            <!--
              ...
              Wave path period = 72px, drawn from x=-144 to x=+216 (4× wide)
            -->
            <g id="bottleWaveY" transform="translate(0,160)">
              ...
              <!-- Wave 2: L→R (slower, opposite phase) -->
    ...
    <div class="vessel-stats">
      ...
      <div class="goal-line">
        ...
        <button class="vol-btn" onclick="toggleVolPanel()">⚙ Editar</button>
      </div>
      <div class="btns">
        <button class="btn btn-add" onclick="addDrop(event)">＋</button>
        <button class="btn btn-remove" onclick="removeDrop()">－</button>
      </div>
    </div>
  </div>
  ...
  <div class="vol-panel" id="volPanel">
    <div class="vol-row">
      <label>Meta diária total (ml)</label>
      <div class="num-wrap">
        <input class="vol-input" id="inputTotalMl" type="number" min="100" max="10000" step="100" value="2000">
        <div class="spin-btns">
          <button class="spin-btn" onclick="spinInput('inputTotalMl', 100)">↑</button>
          <button class="spin-btn" onclick="spinInput('inputTotalMl', -100)">↓</button>
        </div>
      </div>
    </div>
    <div class="vol-row">
      ...
      <div class="num-wrap">
        ...
        <div class="spin-btns">
          <button class="spin-btn" onclick="spinInput('inputContainerMl', 50)">↑</button>
          <button class="spin-btn" onclick="spinInput('inputContainerMl', -50)">↓</button>
        </div>
      </div>
    </div>
    <div class="vol-hint" id="volHint">→ 4 garrafas necessárias</div>
    <button class="vol-apply" onclick="applyVolume()">Aplicar ✓</button>
  </div>
  ...
  <div class="graph-wrap">
    <div class="section-label">últimos 7 dias</div>
  ...
  <div class="streak">
    ...
    <span>Sequência: <b id="streakCount">0</b> dias</span>
  </div>
</div>
...
<script>
  ...
  /* ── FILL & WAVES ──
     Bottle fill area: y=2 (cap top) to y=154 (body bottom) → h=152
     Cup fill area:    y=80 (open top) to y=158 (bottom)   → h=78
     ...
     CSS animation on inner groups only affects translateX → no conflict
  */
  ...
  function updateHint() {
    ...
    const noun  = config.type === 'bottle'
      ...
      : (g === 1 ? 'copo' : 'copos');
    document.getElementById('volHint').textContent = `→ ${g} ${noun} necessários`;
  }
  ...
  function addDrop(e) {
    ...
    if (before >= config.goal) { showToast('🎉 Meta já atingida!'); return; }
    setCount(before + 1);
    render(true);
    if (e) spawnParticles(e);
    if (before + 1 === config.goal)
      setTimeout(() => showToast('💧 Meta atingida! Parabéns!'), 350);
  }
